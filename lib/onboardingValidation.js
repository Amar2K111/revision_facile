  ...
  if (!Array.isArray(answers.weakSubjects) || answers.weakSubjects.length === 0) {
    const weakMsg =
      classId === "term" || classId === "bts2"
        ...
        : "Choisis au moins une matière difficile.";
    return { ok: false, error: weakMsg };
  }

  if (!areWeakSubjectsValid(answers, /** @type {string[]} */ (answers.weakSubjects))) {
    return { ok: false, error: "Sélection invalide pour les matières ou notions." };
  }
